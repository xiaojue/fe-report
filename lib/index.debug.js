...
};_this.config=__WEBPACK_IMPORTED_MODULE_1__utils__["a" /* default */].assignObject(config,options);_this.queue={get:[],post:[]};_this.getUrl=_this.config.url+_this.config.getPath;_this.postUrl=_this.config.url+_this.config.postPath;return _this;}_createClass(Report,[{key:'reportByGet',value:function reportByGet(data,cb){this.sendData('get',data,cb);}},{key:'reportByPost',value:function reportByPost(data,cb){this.sendData('post',data,cb);}},{key:'sendData',value:function sendData(type,data,cb){if(this.catchData(type,data)){this.delayReport(cb);}}},{key:'delayReport',value:function delayReport(cb){var _this2=this;if(!this.trigger('beforeReport'))return;var delay=this.config.mergeReport?this.config.delay:0;setTimeout(function(){if(!_this2.trigger('beforeSend'))return;_this2.report(cb);},delay);}// push数据到pool
